...
function loadData() {
    $.ajax({
        ...
        data: { 'MachineID': $('#Machine').val(), 'FrequencyID': $('#Frequency').val(), 'MaintenanceDate': $('#MaintenanceDate').val(), 'Shift': $('#Shift').val(), 'OperatorID': $('#Operator').val(), 'Month': $('#Month').val(), 'Year': $('#Year').val() },
        type: "GET",
        ...
        success: function (result) {
            ...
            $.each(result.lstMachineMtn, function (key, item) {
                ...
                html += '<td><a href=\'/MachineMtnDaily/Edit6Month?id=' + item.ID + '\'>Edit</a> | <a href="#" onclick="return DeleleByID(\'' + item.ID + '\')">Delete</a></td>';
                html += '</tr>';
            });
            $('.tbody').html(html);
        },
...
function clearTextBox() {
    ...
    //$('#btnUpdate').hide();
    $('#btnAdd').show();
    $('#myModalLabel').text("Add New Machine Maintenance 6 Monthly");
